...
async function callClaude(messages, profile) {
  ...
  if (!apiKey || apiKey === 'your_anthropic_api_key_here') {
    throw new Error('API key not set. Please add REACT_APP_ANTHROPIC_API_KEY to your .env file.');
  }
...
function Chat({ profile }) {
  ...
  const send = useCallback(async (text) => {
    ...
    setLoading(true);
    try {
      const reply = await callClaude(next, profile);
  ...
  return (
    <div className="screen">
      ...
      <div className="card mt12 anim anim-3">
        ...
        {process.env.REACT_APP_ANTHROPIC_API_KEY && process.env.REACT_APP_ANTHROPIC_API_KEY !== 'your_anthropic_api_key_here'
          ? <p className="t-sm" style={{ color:'var(--g-mid)' }}>✓ Claude AI is connected and ready.</p>
          : <p className="t-sm c-muted">Add <code>REACT_APP_ANTHROPIC_API_KEY</code> to your <code>.env</code> file to enable AI chat.</p>
        }
      </div>
